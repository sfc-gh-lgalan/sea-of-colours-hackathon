...
def _harvester_demand(opt: Any) -> int:
    ...
    if kind == "shape":
        ...
        return len({str(m.get("unit")) for m in (pay.get("moves") or []) if isinstance(m, Mapping) and m.get("a") == "drop"})
...
def _ranked_value(opt: Any, agent_view: Mapping[str, Any]) -> float:
    ...
    """
    if str(getattr(opt, "kind", "")) == "shape":
        return float((getattr(opt, "payload", None) or {}).get("value") or 0.0)
...
def _pack_shape(pk: "_Packer", payload: Mapping[str, Any]) -> None:
    """SAGAR_COCO: the verified night, emitted verbatim. Consumes every harvester
    and the probes it lists so nothing else in the plan can double-book a unit."""
    moves = [dict(m) for m in (payload.get("moves") or []) if isinstance(m, Mapping)]
    if not moves:
        pk.log.append("cut SHAPE_PLAN: empty")
        return
    for m in moves:
        if m.get("a") == "probe":
            c = _cell(m.get("at"))
            if c is not None:
                pk._probed_cells.add(c)
            pk.probe_budget = max(0, pk.probe_budget - 1)
        elif m.get("a") == "drop":
            c = _cell(m.get("at"))
            if c is not None:
                pk._drop_cells.add(c)
    pk.moves.extend(moves)
    pk._h_idx = len(pk.harvesters)
    pk.log.append(f"SHAPE_PLAN compiled verbatim ({len(moves)} orders)")


def _pack_chaff(pk: "_Packer", payload: Mapping[str, Any]) -> None:
    """SAGAR_COCO: one flare, one slot, no target. Placed where the plan sits when
    it is packed; the harness re-sequences it after the last pickup."""
    if scorch.stock(pk.agent_view).get("chaff", 0) <= 0 or getattr(pk, "chaff_spent", False):
        pk.log.append("cut chaff: no flare in the rack")
        return
    pk.moves.append({"a": "chaff_flare"})
    pk.chaff_spent = True
    pk.log.append("chaff flare queued (one slot; re-sequenced after the fleet lifts)")
