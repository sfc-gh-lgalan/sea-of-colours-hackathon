...
from sea_of_colours.orchestrator_2.harnesses.sagar_coco._v7.orbit_wishlist import (
    Wishlist,
)
...
from sea_of_colours.orchestrator_2.harnesses.sagar_coco._v7.prompt import (
    format_state_block,
...
def format_rack_block(agent_view: Mapping[str, Any]) -> str:
    ...
    if rack["chaff"] > 0:
        lines.append(
            f"  chaff x{rack['chaff']} — one flare cancels EVERY seat's orders for 3 hours "
            "(you are immune only in the launch hour). Pick CHAFF_JAM off the menu; it is "
            "queued AFTER your last pickup so the jam lands on their landings, not yours."
        )
    lines.append(
        "  -> Weapon options on the menu carry an EXPECTED NET points figure computed by "
        "the harness from what you can see. The one tagged RECOMMENDED clears the bar; "
        "take it unless you can name a concrete reason the pricing is wrong tonight."
    )
    if rack["emp"] > 0:
        lines.append(
...
def _assemble_doctrine(
    *,
    ...
    if scorch.stock(agent_view)["chaff"] > 0:
        text += "\n\n" + doctrine.DOCTRINE_CHAFF_JAM
    text += "\n\n" + doctrine.DOCTRINE_SHAPE
