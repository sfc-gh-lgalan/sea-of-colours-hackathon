...
from sea_of_colours.orchestrator_2.harnesses.sagar_coco import chain_filter
from sea_of_colours.orchestrator_2.harnesses.sagar_coco import comb_shapes
from sea_of_colours.orchestrator_2.harnesses.sagar_coco import option_economics
from sea_of_colours.orchestrator_2.harnesses.sagar_coco import packager
from sea_of_colours.orchestrator_2.harnesses.sagar_coco import scorch
from sea_of_colours.orchestrator_2.harnesses.sagar_coco import value_pyramid
from sea_of_colours.orchestrator_2.harnesses.sagar_coco.seam_control import (
    SeamPattern,
)
from sea_of_colours.game.tuning import probe_vision_radius
from sea_of_colours.orchestrator_2.harnesses.sagar_coco._v7.probe_hints import (
    _grid_dims,
    _vision_disk,
    _visible_red,
)
...
def shape_option(moves: Sequence[Mapping[str, Any]], log: Sequence[str], value: float,
                 red_cells: int, pures: int) -> Optional[Option]:
    """SAGAR_COCO: the verifier's complete night as ONE selectable id.

    The verifier used to override the model in most turns because the menu
    only offered walk-in patterns and the model chose them. Offering the
    verified night on the menu lets the model participate in the choice
    instead of being corrected after it.
    """
    if not moves:
        return None
    units = sorted({str(m.get("unit")) for m in moves if m.get("a") == "drop"})
    probes = sum(1 for m in moves if m.get("a") == "probe")
    return Option(
        option_id="SHAPE_PLAN",
        kind="shape",
        title=f"the verified night: {len(units)} unit(s), {probes} probe(s), {red_cells} RED cell(s), {pures} pure(s), ~{value:.0f} pts",
        detail="passes all eight shape rules; lands ON every visible pure; probes after the fleet",
        execute_lines=[f"SHAPE_PLAN: {len(moves)} orders, compiled verbatim"],
        payload={"moves": [dict(m) for m in moves], "value": value, "unit": "all harvesters"},
        rationale=" ".join(str(x) for x in log)[:600],
    )


def _chaff_option(agent_view: Mapping[str, Any]) -> Optional[Option]:
    """SAGAR_COCO: the second weapon, offered when a flare is in the rack.

    Chaff cancels EVERY seat's orders for three hours (rulebook 4.9.5). The
    launcher is immune only in the launch hour and jams itself for the two
    after, so the play is an egress jam: fire it at the hour your own units
    are already lifted (or have nothing left to do) and the rivals lose the
    hours in which they would have landed on or lifted from the contested
    ground. Costs one slot, no credits, 255 BLUE was paid in orbit.
    """
    rack = scorch.stock(agent_view)
    if rack.get("chaff", 0) <= 0:
        return None
    specs = (agent_view.get("orbit") or {}).get("weapon_specs") or {}
    hours = int((specs.get("chaff") or {}).get("duration_hours") or 3)
    return Option(
        option_id="CHAFF_JAM",
        kind="chaff",
        title=f"chaff flare: cancel every rival order for {hours}h",
        detail=(
            f"ALL seats lose {hours} hours of orders (you are immune only in the launch hour). "
            "Queue it AFTER your own pickups so the jam lands on their late landings and lifts."
        ),
        execute_lines=["CHAFF_JAM: chaff_flare (ONE move, no target, ONE charge; place it after your last pickup)"],
        payload={"kind_note": "chaff", "hours": hours},
        rationale=(
            "Tempo insurance for a contested pure or a final night: rivals who planned to land on "
            "or lift from the seam in the jammed hours lose those hours and anything still on the "
            "surface at dawn is destroyed. It jams you too, so it only pays once your own fleet is "
            "lifted. Wasted when nobody is coming."
        ),
    )
...
def build_registry(
    *,
    ...
    chaff_opt = _chaff_option(agent_view)
    if chaff_opt is not None:
        reg[chaff_opt.option_id] = chaff_opt
...
def _option_drop_cells(opt: Option) -> List[Tuple[int, int]]:
    ...
    if opt.kind in ("probe", "supersede", "emp", "chaff", "shape"):
        return out
    for w in (pay.get("waves") or []):
        if isinstance(w, Mapping):
...
_KIND_HEADERS = [
    ("shape", "THE VERIFIED NIGHT — id SHAPE_PLAN (the harness's own complete plan: every unit, every probe, already checked against the eight shape rules; pick it alone, or pick it plus ONE weapon id)"),
    ("chaff", "CHAFF FLARE — id CHAFF_JAM (you own a flare; ONE hour-slot, cancels EVERY seat's orders for 3h incl. yours after the launch hour; queue it after your last pickup)"),
...
_KIND_BLURB = {
    "shape": "a complete legal night built from the same board you see, valued in points. Choosing it is not giving up: it is the fastest way to a clean night. Choose something else only when you can say what it banks that this does not; the harness will check.",
    "chaff": "egress jam: the rivals lose the hours they needed to land on or lift from the contested ground. Only pays once your own fleet is lifted; wasted when nobody is coming.",
...
def _econ_detail_lines(
    ...
    """
    lines: List[str] = []
    if opt.kind in ("probe", "supersede", "emp", "chaff"):
        pay = opt.payload or {}
        bits: List[str] = []
...
_DEPLOY_KINDS = frozenset({"grab", "blue_grab", "seam", "hotdrop", "chain", "frontier", "shape"})
