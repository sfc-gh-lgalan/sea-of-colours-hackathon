...
from sea_of_colours.orchestrator_2.harnesses.sagar_coco._v7.strategies import (  # noqa: F401
...
# ── SAGAR_COCO additions ─────────────────────────────────────────────
DOCTRINE_CHAFF_JAM = """\
CHAFF (you own a flare). A flare cancels EVERY seat's orders for three hours,
yours included after the launch hour. So it is never an opening move: it is
the last thing you do. Queue CHAFF_JAM after your final pickup, when your
units are already lifted and the rivals still have landings or lifts to make
on the contested seam. Anything they leave on the surface at dawn is destroyed.
Worth it when rival harvesters stand next to your pure, or on the final night
when their eye is on your seam. Wasted when nobody is coming: then keep it."""

DOCTRINE_SHAPE = """\
THE SHAPE OF A GOOD NIGHT (these are checked by the harness before submit):
1. A unit that takes a pure LANDS on it. Never walk to a pure; landing banks it
   at hour one and a walk-in gives a rival the hours to take it first.
2. Two units never share a cell. A harvested cell is GREEN now, and a second
   unit stepping into a partner's wake pays 100 for nothing.
3. Never step on GREEN a rival left behind. It is a liability, not a parcel.
4. Every harvester in orbit deploys. A unit that sits out banks nothing.
5. Tail policy. Alone on a live pure with a graded halo: take the pure and push
   at least three cells into the mass and vein. Two pures under rival eyes:
   land, lift, no detour; tempo beats a vein on the way in.
6. Probes go AFTER the drops, because launches are public and tell the rival
   where you are working. The one exception is the single probe that lights
   a fogged landing, which must come first.
7. On a RIVAL sign you cannot see into: supersede their finder probe first,
   then land as close to the centre as the light allows and comb warmer.
8. On an ordinary seam with no jackpot: long chains. Six distinct RED cells
   across the fleet is the floor.
If your plan breaks one of these, the harness will replace it with its own
shape plan and say so on the card. Plan within the shape and you keep control."""
