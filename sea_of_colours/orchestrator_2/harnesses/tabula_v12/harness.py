"""tabula_v12 — night entry point (clean fork of v11; changes land in Section 2).
...
# ── Agent identity ─────────────────────────────────────────────────────
INNER_AGENT_LABEL = "TABULA_V12"
THINKER_AGENT_LABEL = "TABULA_V12_THINKER"
...
def _split_on() -> bool:
    """v12 runs the contained split by default; ``TABULA_V12_SINGLE=1`` disables."""
    return os.environ.get("TABULA_V12_SINGLE", "0").strip().lower() not in (
        "1", "true", "yes",
    )
...
def _autofill_on() -> bool:
    ...
    AGENT spends the shortfall and stays the author. Set ``TABULA_V12_AUTOFILL=1``
    to restore the old guarantee and measure the two against each other.
    """
    return os.environ.get("TABULA_V12_AUTOFILL", "0").strip().lower() in (
        "1", "true", "yes", "on",
    )
