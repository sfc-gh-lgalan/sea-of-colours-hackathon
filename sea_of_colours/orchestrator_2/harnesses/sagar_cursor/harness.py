"""sagar_cursor — night entry point (clean fork of v11; changes land in Section 2).
...
both on the inference API) is native and on by default; ``SAGAR_CURSOR_SINGLE=1``
...
from sea_of_colours.orchestrator_2.harnesses.sagar_cursor._v7 import (
    chat_schema,
...
from sea_of_colours.orchestrator_2.harnesses.sagar_cursor import prompt as prompt_mod
...
# ── Agent identity ─────────────────────────────────────────────────────
INNER_AGENT_LABEL = "SAGAR_CURSOR"
THINKER_AGENT_LABEL = "SAGAR_CURSOR_THINKER"
...
def _split_on() -> bool:
    """v12 runs the contained split by default; ``SAGAR_CURSOR_SINGLE=1`` disables."""
    return os.environ.get("SAGAR_CURSOR_SINGLE", "0").strip().lower() not in (
        "1", "true", "yes",
    )
...
def _autofill_on() -> bool:
    ...
    AGENT spends the shortfall and stays the author. Set ``SAGAR_CURSOR_AUTOFILL=1``
    to restore the old guarantee and measure the two against each other.
    """
    return os.environ.get("SAGAR_CURSOR_AUTOFILL", "0").strip().lower() in (
        "1", "true", "yes", "on",
    )
