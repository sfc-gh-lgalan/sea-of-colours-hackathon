...
from sea_of_colours.orchestrator_2.harnesses.emp_harvest_test import chain_filter
from sea_of_colours.orchestrator_2.harnesses.emp_harvest_test import comb_shapes
from sea_of_colours.orchestrator_2.harnesses.emp_harvest_test import option_economics
from sea_of_colours.orchestrator_2.harnesses.emp_harvest_test import packager
from sea_of_colours.orchestrator_2.harnesses.emp_harvest_test import scorch
from sea_of_colours.orchestrator_2.harnesses.emp_harvest_test import value_pyramid
from sea_of_colours.orchestrator_2.harnesses.emp_harvest_test import weapon_forge
from sea_of_colours.orchestrator_2.harnesses.emp_harvest_test.seam_control import (
    SeamPattern,
)
...
def build_registry(
    *,
    ...
    # Declared snap/chaff plays (weapon_plays.PLAYS). Built LAST for the same
    # reason SNAP COVER is — their rationale names the grab they pair with, and
    # only the finished menu knows whether tonight actually offers a
    # SMASH_GRAB or a BLIND_AND_GRAB to name. Then moved to the FRONT, because
    # they are hour-one denial like the salvos and a denial play the thinker
    # reads last is one it plans around instead of planning with.
    #
    # EMP is absent from PLAYS by design, so nothing here can shadow the
    # fork's own BLIND_SCORCH — see the header of weapon_plays.py.
    weapon_opts = weapon_forge.build_options(
        agent_view, seam_patterns or (),
        option_cls=Option, present=list(reg),
    )
    if weapon_opts:
        salvos = [(k, v) for k, v in reg.items() if v.kind == "emp"]
        rest = [(k, v) for k, v in reg.items() if v.kind != "emp"]
        reg = OrderedDict(salvos + list(weapon_opts.items()) + rest)
...
# Snap and chaff groups, prepended by the forge from the declared plays. An
# option whose kind has no header renders under nothing, which reads to the
# thinker as a stray id rather than a play — so this has to track PLAYS
# rather than be typed out beside the EMP header above.
...
def _econ_detail_lines(
    ...
    # A weapon banks nothing, so the ordinary yield line prices it at
    # "red ~+0 · blue 0 · green 0" — and the menu explicitly tells the model to
    # rank on that line. Sat next to a grab worth several hundred red, a denial
    # play therefore loses every comparison it is ever in, no matter how good
    # the argument above it is: the seat held a snap on twenty-one planning
    # turns, was offered one on nine of them, and fired zero. Arguing in prose
    # that yield is the wrong axis does not work; the number has to change.
    #
    # `denial_yield` prices the play in what the RIVAL loses instead, over the
    # ground the charge covers. It is only ever present on a weapon option.
    _denial = (opt.payload or {}).get("denial_yield")
    if _denial:
        lines.append(_denial)
    else:
